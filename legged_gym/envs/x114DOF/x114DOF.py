...
class x114DOF(LeggedRobot):
    ...
    def compute_ref_state(self):
        ...
        self.ref_dof_pos[:, 0] = -sin_pos_l * scale_2       #left_hip_pitch
        self.ref_dof_pos[:, 2] = -sin_pos_l * 0.11       #left_hip_yaw
        self.ref_dof_pos[:, 3] = -sin_pos_l * scale_1       #left_knee_pitch
        self.ref_dof_pos[:, 4] = -sin_pos_l * scale_1        #left_ankle_pitch
        # print(phase[0], sin_pos_l[0])
        # right
        sin_pos_r[sin_pos_r < 0] = 0
        self.ref_dof_pos[:, 6] = -sin_pos_r * 0.11
        self.ref_dof_pos[:, 13] = sin_pos_l * 0.11

        self.ref_dof_pos[:, 7] = -sin_pos_r * scale_2
        self.ref_dof_pos[:, 9] =  sin_pos_r * 0.11
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[self.cfg.commands.num_commands: self.cfg.commands.num_commands+self.num_actions] = noise_scales.dof_pos * self.obs_scales.dof_pos
        noise_vec[self.cfg.commands.num_commands+self.num_actions: self.cfg.commands.num_commands+2*self.num_actions] = noise_scales.dof_vel * self.obs_scales.dof_vel
        noise_vec[self.cfg.commands.num_commands+2*self.num_actions: self.cfg.commands.num_commands+3*self.num_actions] = 0.  # previous actions
        noise_vec[self.cfg.commands.num_commands+3*self.num_actions: self.cfg.commands.num_commands+3*self.num_actions + 3] = noise_scales.ang_vel * self.obs_scales.ang_vel   # ang vel
        noise_vec[self.cfg.commands.num_commands+3*self.num_actions + 3: self.cfg.commands.num_commands+3*self.num_actions + 6] = noise_scales.quat * self.obs_scales.quat         # euler x,y
        return noise_vec
